    ...
    return dispatch => {
        ...
        if (!idToken)
        {
            ...
            if (expirationTime < new Date())
            {
                ...
                dispatch(expiresInThisTime((expirationTime.getTime() - new Date().getTime())/1000))
            }
...
export const auth = (email, password , isSignUp) => {
    return dispatch => {
        ...
        let url = "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyDjBJS5bkPCzD0fpJcenkmXPzUzm4XAJiI";
        if (!isSignUp){
            url = "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDjBJS5bkPCzD0fpJcenkmXPzUzm4XAJiI";
        }
        if (DummyData.email !== '')
        {
        axios.post(url, DummyData).then(
            responce => {
                        const expirationTime = new Date(new Date().getTime() + responce.data.expiresIn*1000)
