...
import { Paper_Created_3, Paper_is_creating_3} from '../../store/store/actions/auth';
...
class StudentDetails extends Component {
    ...
    componentDidUpdate()
    {
        ...
        if(this.is_mounted && this.state.saveClicked && !this.state.saveClickedRedirect)
   {     let previousAns = [];
        ...
        //         this.state.controls.rollNo.value,
        ...
        let StudentDetailse = { 
            ...
            {
                axios.post("/Responces/ResponceOf" + this.props.paperId + '.json' ,previousAns[h])
            }
            console.log("Data Post")  
            axios.post("/Responces/ResponceOf" + this.props.paperId + '.json' ,StudentDetailse).then(res =>{
    ...
    render () {
        ...
        let content = 
                        ...
                        <form>
                        {form}
                        <Button click={this.submitHandler} disabled={!this.state.isAllFieldsAreValid}>Save Responce</Button>
                        </form>
                        </div>
   ...
   if(this.props.loading)
    ...
    return (
        <div>
                {content}
                {this.state.saveClickedRedirect ?<Redirect exact to = "/getAnswer"/> : null}
            </div>
